...
all_solution = solve_scp(model, conf.scp_params)

# plot final solution
X = all_solution['state'][-1]
...
contacts = model._contacts 
nu = model._n_u
for contact_idx, contact in enumerate (contacts):
    cop_idx_0 = int(contact_idx*nu/len(contacts))
    plt.rc('text', usetex = True)
    plt.rc('font', family ='serif')
    fig, (copx, copy, fx, fy, fz, tauz) = plt.subplots(6, 1, sharex=True) 
    U = all_solution['control'][-1][cop_idx_0:cop_idx_0+int(nu/len(contacts))] 
