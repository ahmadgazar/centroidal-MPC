# headers
from src.optimizer import Control_optimizer, Slack_optimizer, State_optimizer 
from src.contact_plan import create_contact_trajectory 
...
class Centroidal_model:
    ...
    def __fill_optimizer_indices(self):  
        ...
        state_optimizers = []
        x = ['com_x', 'com_y', 'com_z', 
            'lin_mom_x', 'lin_mom_y', 'lin_mom_z', 'ang_mom_x', 'ang_mom_y', 'ang_mom_z']
        ...
        for contact_idx, contact in enumerate(self._contact_trajectory):
            ...
            control_optimizers = [] 
            for control in u:
                control_optimizers.append(Control_optimizer(control, contact_idx, 
                                    self._robot, self._n_x, self._n_u, self._N))
            if self._robot == 'TALOS':
        ...
        # state slack indices
        self._state_slack_optimizers_indices = Slack_optimizer('state', self._n_x, self._n_u, 
                                                                         self._n_t, self._N)
    ...
    def integrate_model_one_step(self, x, u, contacts_position_all, contacts_logic_all, contacts_orientation_all):
        ...
        def dynamics_loop_body(contact_idx, curr):
            ...
            curr = curr.at[3:6].add(lin_mom_per_contact) 
            curr = curr.at[6:9].add(ang_mom_per_contact) 
            return curr 
    ...
    def integrate_dynamics_trajectory(self, traj_tuple):
        ...
        def loop_body(time_idx, f_traj):
            f = self.integrate_model_one_step(X[:, time_idx], U[:,time_idx], 
                        self._contact_data['contacts_position'][time_idx,:], 
                          self._contact_data['contacts_logic'][time_idx, :], 
                      self._contact_data['contacts_orient'][time_idx,:,:,:])
            f_traj = f_traj.at[:, time_idx].set(f) 
            return f_traj
    ...
    def compute_trajectory_data(self, traj_tuple):
        ...
        def loop_body(time_idx, traj_data):
            f, A, B, C, K, Sigma_next, dSigma_dx_cur_next, dSigma_du_curr_next = self.compute_everything(X[:, time_idx], U[:,time_idx],
                                self._contact_data['contacts_position'][time_idx,:], self._contact_data['contacts_logic'][time_idx, :],
                                                self._contact_data['contacts_orient'][time_idx,:,:,:], traj_data['Covs'][time_idx,:,:]) 
            ...
            traj_data['dynamics'] = traj_data['dynamics'].at[:, time_idx].set(f)
            traj_data['gradients']['f_x'] = traj_data['gradients']['f_x'].at[time_idx, :,:].set(A)
            traj_data['gradients']['f_u'] = traj_data['gradients']['f_u'].at[time_idx, :,:].set(B)
            traj_data['gradients']['f_w'] = traj_data['gradients']['f_w'].at[time_idx, :,:].set(C)
            traj_data['LQR_gains'] = traj_data['LQR_gains'].at[time_idx, :,:].set(K)
            traj_data['Covs'] = traj_data['Covs'].at[time_idx+1, :,:].set(Sigma_next)
            traj_data['Covs_gradients']['Cov_dx'] = traj_data['Covs_gradients']['Cov_dx'].at[time_idx+1,:,:,:,:].set(dSigma_dx_prev_next)
            traj_data['Covs_gradients']['Cov_du'] = traj_data['Covs_gradients']['Cov_du'].at[time_idx+1,:,:,:,:].set(dSigma_du_prev_next)
            traj_data['Covs_gradients']['Cov_dx'] = traj_data['Covs_gradients']['Cov_dx'].at[time_idx+1,:,:,:, time_idx].set(dSigma_dx_cur_next)
            traj_data['Covs_gradients']['Cov_du'] = traj_data['Covs_gradients']['Cov_du'].at[time_idx+1,:,:,:, time_idx].set(dSigma_du_curr_next)
            return traj_data 
