...
class WholeBodyModel:
        ...
        for frame_idx in support_feet_ids:
            ...
            if forceTask is not None:
                if frame_idx == self.rfFootId:
                    spatial_force_des = self.rdata.oMf[frame_idx].actInv(pinocchio.Force(forceTask[0:3], np.zeros(3)))
                if frame_idx == self.lfFootId:
                    spatial_force_des = self.rdata.oMf[frame_idx].actInv(pinocchio.Force(forceTask[3:6], np.zeros(3)))
                if frame_idx == self.rhFootId:
                    spatial_force_des = self.rdata.oMf[frame_idx].actInv(pinocchio.Force(forceTask[6:9], np.zeros(3)))
                if frame_idx == self.lhFootId:
                    spatial_force_des = self.rdata.oMf[frame_idx].actInv(pinocchio.Force(forceTask[9:12], np.zeros(3)))
                ...
                cost.addCost(rmodel.frames[frame_idx].name +"contactForceTrack", force_track, forceTrackWeight)
    ...
    def add_stat_ctrl_reg_costs(self, cost, preImpact):
        ...
        # higher penalization  of states and controls at time of impact (check the config file for task weights)
        if preImpact:
    ...
    def add_terminal_costs(self, feetPos):
        ...
        terminalCostModel = self.createSwingFootModel(supportFeetIds, 
                    swingFootTask=swingFootTask, comTask=self.X[:3, -1], 
                 centroidalTask=self.X[3:9, -1],forceTask=self.U[:, -1])                            
    ...
    def createPaceShootingProblem(self):
        ...
        if robot_name == 'solo':
            ...
            self.comRef = (rfFootPos0 + rhFootPos0 + lfFootPos0 + lhFootPos0) / 4
        # elif robot_name == 'bolt' or robot_name == 'bolt_humanoid':
        elif self.rmodel.name == 'talos':
            # print('yes')
            rfFootPos0 = rdata.oMf[self.rfFootId].translation
            lfFootPos0 = rdata.oMf[self.lfFootId].translation
        ...
        for gait in self.gait_templates:
            for phase in gait:
                if robot_name == 'solo' and phase == 'doubleSupport':
                    ...
                    loco3dModel += self.createSingleSupportFootstepModels([rfFootPos0], [self.lfFootId], [self.rfFootId])
                elif phase == 'lflhStep':
                    loco3dModel += self.createSingleSupportFootstepModels([lfFootPos0, lhFootPos0], 
                                                                    [self.rfFootId, self.rhFootId], 
                                                                    [self.lfFootId, self.lhFootId])
                elif phase == 'lfStep':
                    loco3dModel += self.createSingleSupportFootstepModels([lfFootPos0], [self.rfFootId], [self.lfFootId])                                                                
        if robot_name == 'solo' and self.TRACK_CENTROIDAL:
            loco3dModelTerminal = self.add_terminal_costs([lfFootPos0, rfFootPos0, lhFootPos0, rhFootPos0])
        # elif (robot_name == 'bolt' or robot_name == 'bolt_humanoid') and self.TRACK_CENTROIDAL:
        elif self.rmodel.name == 'talos' and self.TRACK_CENTROIDAL:
            loco3dModelTerminal = self.add_terminal_costs([lfFootPos0, rfFootPos0])    
        else:
        ...
        for _ in range(supportKnots):
            ...
            if self.TRACK_CENTROIDAL:
                ...
                doubleSupportModel += [self.createSwingFootModel(supportFeetIds, 
                             swingFootTask=swingFootTask, forceTask=forceTask)]
                self.time_idx += 1
            else:
